...
export function useAIPrediction(modelKey = "fast") {
  ...
  const activeModelId = isNoneMode ? null : (MODELS[normalizedModelKey] ?? MODELS.fast);
  ...
  useEffect(() => {
    const isModelSwitch = prevModelIdRef.current !== activeModelId;
    ...
    // Auto-start download when user explicitly switches models (skip for "none")
  ...
  const initEngine = useCallback(async (force = false) => {
    if (isNoneMode) return;                       // no engine needed
  ...
  const predict = useCallback(async (words, langCode = "en", categoryId, tapContext, transcript, messageHistory = [], gender = "male", emotionOverride = null, corePrefixWords = []) => {
    ...
    try {
      // ── Mode B: Contextual reply when transcript is available ──────────
